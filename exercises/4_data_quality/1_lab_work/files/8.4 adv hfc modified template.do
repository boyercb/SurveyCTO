...
version 10
clear
cd "???"             // REPLACE ??? with the directory
use "???", clear     // REPLACE ??? with the data set
...
generate incomplete = missing("???")          // REPLACE ??? with end time variable
...
duplicates tag ???, generate(dup)            // REPLACE ??? with id variable
...
sort id
list id starttime r_enum if ??? == 0 & inlist(0, missing(r_cont),  /// replace ??? with consent variable
                                                 missing(r_count), ///
											     missing(r_bin),   ///
											     missing(r_cat))
...
/*
  ...
  However, in this survey we didn't use any hard range limits for r_cont...
  probably a mistake. We'll check for outliers by looking for
  values that are 3 standard deviations from the mean.
...
egen mean = mean(???)                   // replace ??? with r_cont
egen sd = sd(???)                       // replace ??? with r_cont
generate sds = (??? - mean) / sd        // replace ??? with r_cont
...
// < insert code here to drop outliers > !!!!!!!!!!!!!!!!!!!!!!!!!
...
regress ??? i.r_enum   // replace ??? with variable of interest
regress ??? i.r_enum   // replace ??? with variable of interest
regress ??? i.r_enum   // replace ??? with variable of interest
regress ??? i.r_enum   // replace ??? with variable of interest
